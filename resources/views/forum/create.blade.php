<x-app-layout>
    <div class="max-w-3xl mx-auto px-4 py-8">
        <a href="{{ route('forum.index') }}" class="inline-flex items-center text-sm font-bold text-gray-500 hover:text-brand-dark mb-6 group transition-colors">
        ...
        <div class="bg-white rounded-3xl p-8 border border-gray-100 shadow-sm">
            <div class="mb-8">
                <h1 class="text-2xl font-extrabold text-brand-dark">Mulai Diskusi Baru</h1>
                <p class="text-sm font-medium text-gray-500 mt-1">Sampaikan gagasan, pertanyaan, atau topik menarik untuk dibahas bersama komunitas perpustakaan.</p>
            </div>
            ...
            <form action="{{ route('forum.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                <div>
                    ...
                    <input type="text" name="title" id="title" required placeholder="Contoh: Diskusi mendalam bumi manusia..."
                ...
                <div>
                    ...
                    <textarea name="content" id="content" rows="8" required placeholder="Tuliskan gagasan utamamu di sini..."
                        class="w-full px-4 py-3 rounded-xl border border-gray-200 focus:border-brand-primary focus:ring focus:ring-brand-primary/20 transition-all shadow-sm resize-y text-sm text-brand-dark"></textarea>
                    @error('content')
                        <p class="mt-1 text-xs text-red-500 font-medium">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    <label for="image" class="block text-xs font-bold text-gray-700 uppercase tracking-wide mb-2">Lampirkan Gambar (Opsional)</label>
