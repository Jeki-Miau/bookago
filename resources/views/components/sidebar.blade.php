    ...
    <nav class="flex-1 overflow-y-auto py-6 px-4">
        <ul>
            ...
            <li class="mb-2">
                ...
                <a href="{{ route('home') }}"
                    class="flex items-center px-3 py-2.5 rounded-xl transition {{ request()->routeIs('home') ? 'text-brand-lime bg-gray-50 font-bold shadow-sm border border-gray-100' : 'text-gray-500 hover:text-brand-dark hover:bg-gray-50 font-semibold' }}">
                ...
                <a href="{{ route('books.saved') }}" x-data="{
                        count: 0,
                        init() {
                    ...
                    class="flex items-center px-3 py-2.5 mt-1 rounded-xl transition {{ request()->routeIs('books.saved') ? 'text-brand-lime bg-gray-50 font-bold shadow-sm border border-gray-100' : 'text-gray-500 hover:text-brand-dark hover:bg-gray-50 font-semibold' }}">
                    <svg class="w-5 h-5 mr-3 {{ request()->routeIs('books.saved') ? 'text-brand-lime' : 'text-gray-400' }}"
            ...
            <li class="mt-8 mb-2">
                ...
                <div class="space-y-1">
                    ...
                    <a href="{{ route('home', ['category' => 'Biografi']) }}"
                        class="flex items-center px-3 py-2.5 rounded-xl text-sm font-semibold text-gray-500 hover:text-brand-dark hover:bg-gray-50 transition"><span
            ...
            <li class="mt-8 mb-2">
                ...
                <div class="space-y-1">
                    <a href="{{ route('forum.index') }}"
                        class="flex items-center px-3 py-2.5 rounded-xl text-sm transition group {{ request()->routeIs('forum.*') ? 'text-brand-lime bg-gray-50 font-bold shadow-sm border border-gray-100' : 'font-semibold text-gray-500 hover:text-brand-dark hover:bg-gray-50' }}">
                        <svg class="w-5 h-5 mr-3 {{ request()->routeIs('forum.*') ? 'text-brand-lime' : 'text-gray-400 group-hover:text-blue-500' }}"
                            fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <li class="mt-8 mb-2">
                ...
                <div class="space-y-2">
                    ...
                    <div class="px-3 py-2.5 rounded-xl bg-yellow-50 border border-yellow-100">
                        <p class="text-xs font-bold text-yellow-800 line-clamp-1">{{ $announcement->title }}</p>
                        <p class="text-[10px] text-yellow-600 mt-1 line-clamp-2">{{ $announcement->content }}</p>
                    </div>
                    @endforeach
                </div>
            </li>
