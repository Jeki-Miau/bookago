@props(['title' => 'Admin Panel - BOOKAGO'])
            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div class="flex justify-between h-16">
                    <div class="flex items-center">
                        ...
                        <a href="{{ route('landing') }}" class="flex items-center space-x-2 group shrink-0">
                    ...
                    <div class="flex items-center space-x-2 sm:space-x-4">
                        <div class="hidden md:flex items-center space-x-4 mr-2">
                            <a href="{{ route('landing') }}" class="text-sm text-gray-600 hover:text-brand-lime font-semibold transition">Landing</a>
                            <a href="{{ route('home') }}" class="text-sm text-gray-600 hover:text-brand-lime font-semibold transition">Katalog</a>
                        </div>
                        <div class="flex items-center space-x-2 bg-gray-50 px-2 sm:px-3 py-1.5 rounded-full shrink-0">
                            <img class="w-7 h-7 sm:w-8 sm:h-8 rounded-full border-2 border-brand-lime shadow-sm" src="{{ auth()->user()->avatar ?? 'https://ui-avatars.com/api/?name=' . urlencode(auth()->user()->name) . '&background=0f172a&color=fff' }}" alt="">
                            <span class="text-sm font-bold text-brand-dark hidden sm:block">{{ auth()->user()->name ?? 'Admin' }}</span>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="flex flex-1 relative">
            ...
            <div :class="{'translate-x-0': sidebarOpen, '-translate-x-full': !sidebarOpen}" class="w-64 bg-white shadow-sm border-r border-gray-200 md-sidebar-fixed fixed top-0 inset-y-0 left-0 h-screen overflow-y-auto z-40 transition-transform duration-300 md:translate-x-0">
                ...
                <nav class="mt-6 px-4 space-y-1">
                    <a href="{{ route('admin.dashboard') }}" class="flex items-center px-4 py-3 rounded-xl transition {{ request()->routeIs('admin.dashboard') ? 'bg-brand-lime/10 text-brand-lime font-bold' : 'text-gray-600 hover:bg-gray-50 hover:text-brand-dark' }}">
                        <svg class="w-5 h-5 mr-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zM14 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zM14 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z"></path>
                        </svg>
                        Dashboard
                    </a>
                    <a href="{{ route('admin.announcements') }}" class="flex items-center px-4 py-3 rounded-xl transition {{ request()->routeIs('admin.announcements*') ? 'bg-brand-lime/10 text-brand-lime font-bold' : 'text-gray-600 hover:bg-gray-50 hover:text-brand-dark' }}">
                        <svg class="w-5 h-5 mr-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5.882V19.24a1.76 1.76 0 01-3.417.592l-2.147-6.15M18 13a3 3 0 100-6M5.436 13.683A4.001 4.001 0 017 6h1.832c4.1 0 7.625-1.234 9.168-3v14c-1.543-1.766-5.067-3-9.168-3H7a3.988 3.988 0 01-1.564-.317z"></path>
                        </svg>
                        Pengumuman
                    </a>
                    <a href="{{ route('admin.messages') }}" class="flex items-center px-4 py-3 rounded-xl transition {{ request()->routeIs('admin.messages*') ? 'bg-brand-lime/10 text-brand-lime font-bold' : 'text-gray-600 hover:bg-gray-50 hover:text-brand-dark' }}">
                        <svg class="w-5 h-5 mr-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"></path>
                        </svg>
                        Pesan Masuk
                    </a>
                    <a href="{{ route('admin.warnings') }}" class="flex items-center px-4 py-3 rounded-xl transition {{ request()->routeIs('admin.warnings*') ? 'bg-brand-lime/10 text-brand-lime font-bold' : 'text-gray-600 hover:bg-gray-50 hover:text-brand-dark' }}">
                        <svg class="w-5 h-5 mr-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"></path>
                        </svg>
                        Peringatan
                    </a>
                    <a href="{{ route('admin.users') }}" class="flex items-center px-4 py-3 rounded-xl transition {{ request()->routeIs('admin.users*') ? 'bg-brand-lime/10 text-brand-lime font-bold' : 'text-gray-600 hover:bg-gray-50 hover:text-brand-dark' }}">
                        <svg class="w-5 h-5 mr-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z"></path>
                        </svg>
                        Users
                    </a>
                    <a href="{{ route('admin.discussions') }}" class="flex items-center px-4 py-3 rounded-xl transition {{ request()->routeIs('admin.discussions*') ? 'bg-brand-lime/10 text-brand-lime font-bold' : 'text-gray-600 hover:bg-gray-50 hover:text-brand-dark' }}">
                        <svg class="w-5 h-5 mr-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 8h2a2 2 0 012 2v6a2 2 0 01-2 2h-2v4l-4-4H9a1.994 1.994 0 01-1.414-.586m0 0L11 14h4a2 2 0 002-2V6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2v4l.586-.586z"></path>
                        </svg>
                        Discussions
                    </a>
                    <a href="{{ route('forum.index') }}" class="flex items-center px-4 py-3 rounded-xl transition text-gray-600 hover:bg-gray-50 hover:text-brand-dark mt-4 border-t border-gray-100 pt-4">
                         <svg class="w-5 h-5 mr-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
