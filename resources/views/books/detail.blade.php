<x-app-layout>
    ...
    <div class="min-h-screen bg-gradient-to-b from-gray-50 to-white pb-20">
        ...
        <div class="relative overflow-hidden">
            ...
            <div class="absolute inset-0 opacity-30" style="background-image: url('{{ $coverLarge }}'); background-size: cover; background-position: center; filter: blur(40px) saturate(150%); transform: scale(1.2);"></div>
            ...
            <div class="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-12 pb-8">
                <!-- Back Button -->
                <a href="{{ url()->previous() }}" class="inline-flex items-center text-gray-500 hover:text-brand-primary transition mb-8 group">
                    ...
                    <div class="lg:col-span-2 space-y-8 pt-8">
                        ...
                        <div>
                            <div class="flex flex-wrap gap-2 mb-4">
                                @foreach(array_slice($bookData['categories'], 0, 3) as $category)
                            ...
                            <p class="text-xl text-gray-600 font-medium">
                                Oleh <span class="text-brand-primary">{{ $author }}</span>
                            </p>
                        </div>
                        ...
                        <div class="flex flex-wrap items-center gap-6 py-6 border-y border-gray-100">
                            ...
                            <div class="flex items-center gap-2">
                                <div class="flex">
                                    @for($i = 1; $i <= 5; $i++)
                                    <svg class="w-5 h-5 {{ $i <= round($bookData['averageRating']) ? 'text-yellow-400' : 'text-gray-300' }}" fill="currentColor" viewBox="0 0 20 20">
                                        <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/>
                                    </svg>
                                    @endfor
                                </div>
                                <span class="text-lg font-bold text-gray-800">{{ number_format($bookData['averageRating'], 1) }}</span>
                                <span class="text-sm text-gray-400">({{ number_format($bookData['ratingsCount']) }} ulasan)</span>
                            </div>
                            @endif
                            ...
                            <div class="flex items-center gap-2 text-gray-500">
                                ...
                                <span class="font-medium">{{ $bookData['pageCount'] }} halaman</span>
                            </div>
                            @endif
                            ...
                            <div class="flex items-center gap-2 text-gray-500">
                                ...
                                <span class="font-medium">{{ $bookData['publisher'] }}</span>
                            </div>
                            @endif
                            ...
                            <div class="flex items-center gap-2 text-gray-500">
                                ...
                                <span class="font-medium">{{ $bookData['publishedDate'] }}</span>
                            </div>
                            @endif
                        </div>
                        ...
                        <div class="prose prose-lg max-w-none">
                            ...
                            <div class="text-gray-600 leading-relaxed text-justify">
                                {!! $bookData['description'] ?: 'Deskripsi tidak tersedia untuk buku ini.' !!}
                            </div>
                        </div>
                        ...
                        <div class="bg-gray-50 rounded-2xl p-6 border border-gray-100">
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                ...
                                <div class="flex items-center gap-3 bg-white px-4 py-3 rounded-xl border border-gray-100">
                                    <span class="text-xs font-bold text-gray-400 uppercase">{{ $identifier['type'] }}</span>
                                    <span class="font-mono text-sm text-gray-700">{{ $identifier['identifier'] }}</span>
                                </div>
                                @endforeach
                            </div>
                        </div>
...
async function saveBook(slug) {
    ...
    const bookData = {
        slug: slug,
        title: '{{ addslashes($bookData["title"]) }}',
